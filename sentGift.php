...
$recievedGifts = $conn->query('SELECT user.name, gift.title, gift.giftImage, gift.description, sender_userId FROM userHasGift JOIN user ON user.email = userHasGift.userId JOIN gift ON gift.title = userHasGift.giftID')->fetchAll();

?><h1>Sent Gifts</h1><?php
...
foreach ($recievedGifts as $recievedGift) {
  ...
  <section class="message">
    ...
    <h2>To: <?php echo $recievedGift['name']; ?></h2>
